...
import { RadioGroup, RadioGroupItem } from '@/components/ui/radio-group';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Copy, MessageCircle, CreditCard, Upload, CheckCircle, ArrowLeft, QrCode } from 'lucide-react';
import SEO from '@/components/SEO';
import Logo from '@/components/Logo';
import { useToast } from '@/hooks/use-toast';
import { useMutation } from '@tanstack/react-query';
...
export default function OrderSummaryPage() {
  ...
  const [screenshot, setScreenshot] = useState(null);
  const [isUploading, setIsUploading] = useState(false);
  const [currentStep, setCurrentStep] = useState(1); // 1: Form, 2: Payment, 3: Upload
  const { toast } = useToast();
      ...
      <div className="container mx-auto px-4 py-12 sm:py-20">
              ...
              <CardContent className="space-y-6">
                ...
                {selectedPaymentMethod && (
                  <div className="space-y-4">
                    ...
                    <Select onValueChange={setSelectedPaymentOption} value={selectedPaymentOption}>
                      <SelectTrigger>
        ...
        {currentStep === 3 && (
          <div className="max-w-2xl mx-auto">
            <Card className="bg-white/95 backdrop-blur-lg border-2 border-white/40 shadow-2xl">
              <CardHeader>
                <CardTitle className="text-xl font-black text-green-600">📸 Upload Payment Proof</CardTitle>
                <CardDescription>
              ...
              <CardContent className="space-y-6">
                  ...
                  >
                    {screenshot ? (
                      <>
                        ...
                        />
                        <p className="text-sm text-green-600 font-medium">Screenshot selected</p>
                      </>
                    ) : (
                      <>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  <p className="text-sm text-blue-700">
                    Uploading the payment screenshot is required to verify your payment. 
                    Our team will review and process your order within 24 hours.
