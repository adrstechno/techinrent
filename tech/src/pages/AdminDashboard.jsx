...
import { LogOut, MessageSquare, Calendar, Users, DollarSign, CheckCircle, Mail, Phone, Briefcase, Clock, Eye } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const contactMessagesQuery = useQuery({
    ...
    queryFn: () => fetcher("/api/admin/contacts/", "Failed to fetch contact messages"),
    onSuccess: (data) => {
      setStats(prev => ({
        ...prev,
  ...
  const providerSignupsQuery = useQuery({
    ...
    queryFn: () => fetcher("/api/admin/inquiries/", "Failed to fetch provider signups"),
    onSuccess: (data) => {
      setStats(prev => ({
        ...prev,
  ...
  const linkedinOrdersQuery = useQuery({
    ...
    queryFn: () => fetcher("/api/admin/linkedin-orders/", "Failed to fetch LinkedIn orders"),
    onSuccess: (data) => {
      setStats(prev => ({
        ...prev,
  ...
  const updateLinkedInOrderStatus = async (id, status) => {
    await fetch(`/api/admin/linkedin-orders/${id}/update-status`, {
      method: 'PATCH',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      <SEO title="Admin Dashboard - TechInRent" description="Admin dashboard for managing TechInRent platform" noIndex={true} />
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <StatCard title="Contact Messages" count={stats.totalContacts} unread={stats.unreadContacts} icon={<MessageSquare className="h-8 w-8 text-blue-600" />} />
          <StatCard title="Demo Requests" count={stats.totalDemos} unread={stats.unreadDemos} icon={<Calendar className="h-8 w-8 text-green-600" />} />
          <StatCard title="Provider Signups" count={stats.totalProviders} unread={stats.unreadProviders} icon={<Users className="h-8 w-8 text-purple-600" />} />
          <StatCard title="LinkedIn Orders" count={stats.totalLinkedInOrders} unread={stats.unreadLinkedInOrders} icon={<DollarSign className="h-8 w-8 text-orange-600" />} />
        </div>
