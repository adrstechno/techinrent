...
//       Authorization: `Bearer ${localStorage.getItem("adminToken")}`, // Add auth token
//     },
//     credentials: "include",
//   };
...
//         "GET",
//         "https://tech-in-rent.onrender.com/api/forms"
//       );
...
//   // Fetch all responses once to compute per-form counts for better UX
//   const { data: allResponsesData = [] } = useQuery({
//     queryKey: ["all-responses-summary"],
//     queryFn: async () => {
//       const res = await apiRequest(
//         "GET",
//         "https://tech-in-rent.onrender.com/api/admin/all-responses"
//       );
//       return Array.isArray(res?.data) ? res.data : [];
//     },
//   });

//   // Build a map of Form ObjectId (string) -> submissions count
...
//   // Auto-select the most recent form once forms load
//   useEffect(() => {
//     if (!activeFormId && allForms.length > 0) {
//       // Prefer the first form that has submissions, else fallback to the newest
//       const formWithData = allForms.find(
//         (f) => formObjectIdToCount[String(f._id)] > 0
//       );
//       const target = formWithData || allForms[0];
//       const id = String(target.formId);
//       if (isValidFormId(id)) setActiveFormId(id);
//     }
...
//     queryFn: () => {
//       console.log("Calling API with formId:", debouncedFormId);
//       return apiRequest(
//         "GET",
//         `https://tech-in-rent.onrender.com/api/admin/responses/${debouncedFormId}`
//       );
//     },
...
//     mutationFn: () =>
//       apiRequest("POST", "https://tech-in-rent.onrender.com/api/forms/create"),
//     onSuccess: (data) => {
...
//       apiRequest(
//         "PUT",
//         `https://tech-in-rent.onrender.com/api/admin/responses/${id}/read`
//       ),
//     onSuccess: () => {
//       queryClient.invalidateQueries({
//         queryKey: ["secure-form-submissions", debouncedFormId],
...
//                   } else {
//                     alert("Form link not generated yet.");
//                   }
//                 }}
//                 className="bg-white border-gray-300"
//               >
...
export default function SecureFormAdmin() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const form = allForms.find((f) => f.formId === debouncedFormId);
      if (!form) {
        throw new Error("Form not found");
      }
      const res = await apiRequest(
        "GET",
        `${API_URI}/api/admin/responses/${form._id}`
      );
      return Array.isArray(res.data) ? res.data : [];
    },
    enabled: isValidFormId(debouncedFormId),
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <CardContent>
            <div className="flex flex-col md:flex-row gap-4">
              <Select
                value={isValidFormId(activeFormId) ? activeFormId : ""}
                onValueChange={(val) => {
                  if (isValidFormId(val)) setActiveFormId(val);
                }}
                disabled={isLoadingForms}
              >
                <SelectTrigger className="w-full md:w-[250px] bg-white border-gray-300">
        ...
        {submissionsError && (
          <Card className="bg-white border-red-300">
            <CardContent className="py-4">
              <p className="text-red-600">Error: {submissionsError.message}</p>
              <p className="text-gray-500">
                {submissionsError.message === "Form not found"
